...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	try {
		if (!empty($title) && !empty($url)) {
			...
			$stmt = $mysqli->prepare("INSERT INTO messages (title, url) VALUES (?, ?)");
		...
		<div class="py-10">
			<header class="mb-6 text-center">
				<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					<h2 class="text-gray-500 mt-3">Send a <b>title</b> and a <b>link</b> of the post with the issue.</h2>
				</div>
			</header>
				...
				<form action="/support.php" method="post">
					<div class="shadow sm:rounded-md sm:overflow-hidden">
						<div class="bg-white py-6 px-4 space-y-6 sm:p-6">
							<div>
								<p class="mt-1 text-sm text-gray-500">We will review the link and fix it for you.</p>
							</div>
							...
							<div class="grid grid-cols-6 gap-6">
								...
								<div class="col-span-6">
									...
									<input required type="text" name="url" id="url" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
								</div>
							</div>
							<?php
							if ($message !== "") {
